...
export function FeaturesSection() {
    return (
        ...
        <section id="features" className="py-24 bg-muted/30 border-t border-border relative overflow-hidden transition-colors duration-300">
            ...
            <div className="max-w-[1280px] mx-auto px-6 md:px-16 relative z-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {features.map((feature, index) => (
                        <div
                            key={index}
                            className="bg-card border border-border p-8 rounded-xl shadow-sm hover:shadow-md hover:border-primary/40 transition-all duration-300 group hover:-translate-y-1 animate-in fade-in zoom-in-95 fill-mode-both"
                            style={{ animationDelay: `${index * 100}ms` }}
                        >
                            <div className="w-12 h-12 bg-primary/10 border border-primary/20 rounded-lg flex items-center justify-center mb-6 group-hover:bg-primary/20 transition-colors duration-300 shadow-sm">
