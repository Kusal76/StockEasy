...
export async function POST(req: Request) {
    try {
        const { email, ownerName, shopName, applicationId } = await req.json();
        ...
        if (!settings || !settings.smtp_host || !settings.smtp_pass) {
            return NextResponse.json({ error: "SMTP gateway is not configured." }, { status: 500 });
        }
        ...
        await transporter.sendMail({
            from: `"${settings.platform_name} Onboarding" <${settings.smtp_user}>`,
            to: email,
