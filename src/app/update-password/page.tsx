...
import { Lock, EyeOff, Eye, CheckCircle2, AlertCircle, Loader2, ArrowLeft } from "lucide-react";
...
export default function UpdatePasswordPage() {
    ...
    // Added separate toggles for both fields
    ...
    // Run a quick check on mount to see if the link is already broken/expired
    useEffect(() => {
        const checkSession = async () => {
            const { data: { session } } = await supabase.auth.getSession();
            // If there's no session and no hash in the URL, the link is definitely broken.
            if (!session && !window.location.hash) {
                setErrorMsg("Your secure link has expired or is invalid. Please request a new password reset link.");
            }
        };
        checkSession();
    }, []);
    ...
    const handleUpdate = async (e: React.FormEvent) => {
        ...
        try {
            // Safety Check: Ensure the temporary session actually exists before attempting to update
            const { data: { session } } = await supabase.auth.getSession();
            if (!session) {
                throw new Error("Your password reset link has expired. Please request a new one.");
            }

            // Supabase automatically uses the secure token from the email link URL
            ...
            // Automatically sign them out so they have to use the new password to log in
        ...
        <div className="min-h-screen bg-background flex items-center justify-center p-6 transition-colors duration-300">
            ...
            <header className="fixed top-0 left-0 w-full h-20 border-b border-border bg-background/90 backdrop-blur-md z-50 flex items-center px-8 lg:px-16 transition-colors">
                <Link href="/" className="inline-block hover:opacity-80 transition-opacity">
                    {/* Dual Logo Setup to fix Aspect Ratio and Theme Switching */}
