...
export default function TenantProfilePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 animate-in fade-in duration-500 space-y-6 sm:space-y-8 pb-20 transition-colors">

            {/* Header section */}
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between pb-4 border-b border-border gap-4">
                <div className="flex items-center gap-3 sm:gap-4 w-full sm:min-w-0">
                    <Link
                        href="/admin/shops"
                        className="w-9 h-9 sm:w-10 sm:h-10 shrink-0 bg-card border border-border rounded-xl flex items-center justify-center text-muted-foreground hover:text-foreground hover:border-primary transition-colors cursor-pointer shadow-sm"
                    >
                        <ArrowLeft className="w-4 h-4 sm:w-5 sm:h-5" />
                    </Link>
                    <div className="min-w-0 flex-1">
                        <h1 className="text-xl sm:text-2xl font-bold text-foreground tracking-tight flex flex-wrap items-center gap-2 sm:gap-3 leading-tight">
                            <span className="truncate max-w-[280px] sm:max-w-none">{shopData.name || "Unnamed Shop"}</span>
                            {shopData.status === "ACTIVE" ? (
                                <span className="bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider shadow-sm shrink-0">Active</span>
                            ) : isRejected ? (
                                <span className="bg-muted text-muted-foreground border border-border px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider shadow-sm flex items-center gap-1 shrink-0"><XCircle className="w-3 h-3" /> Rejected</span>
                            ) : (
                                <span className="bg-destructive/10 text-destructive border border-destructive/20 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider shadow-sm shrink-0">{shopData.status}</span>
                            )}
                        </h1>
                ...
                <div className="lg:col-span-1 space-y-6 sm:space-y-8">
                    ...
                    <div className="bg-card border border-border rounded-xl overflow-hidden shadow-sm">
                        ...
                        <div className="p-4 sm:p-5 space-y-4">
                            <div>
                                <p className="text-[10px] sm:text-xs text-muted-foreground uppercase tracking-wider mb-1 font-bold">Business Type</p>
                                <p className="text-xs sm:text-sm font-bold text-foreground">{shopData.business_type || "N/A"}</p>
                            </div>
                            <div>
                                <p className="text-[10px] sm:text-xs text-muted-foreground uppercase tracking-wider mb-1 font-bold">PAN Number</p>
                                <p className="text-xs sm:text-sm font-mono text-foreground uppercase font-medium">{shopData.pan_number || "N/A"}</p>
                            </div>
                            <div>
                                <p className="text-[10px] sm:text-xs text-muted-foreground uppercase tracking-wider mb-1 font-bold">GST Number</p>
                                <p className="text-xs sm:text-sm font-mono text-foreground uppercase font-medium">{shopData.gst_number || "N/A"}</p>
                            </div>
                            <div>
                                <p className="text-[10px] sm:text-xs text-muted-foreground uppercase tracking-wider mb-1 font-bold">Drug License</p>
                                <p className="text-xs sm:text-sm font-mono text-foreground uppercase font-medium break-all whitespace-normal">{shopData.license_number || "N/A"}</p>
                            </div>
                            <div>
                                <p className="text-[10px] sm:text-xs text-muted-foreground uppercase tracking-wider mb-1 font-bold">Business Email</p>
                                <p className="text-xs sm:text-sm text-foreground font-medium break-all whitespace-normal">{shopData.email_address || "N/A"}</p>
                            </div>
                            <div>
                                <p className="text-[10px] sm:text-xs text-muted-foreground uppercase tracking-wider mb-1 font-bold">Contact Number</p>
                                <p className="text-xs sm:text-sm text-foreground font-medium">{shopData.contact_number || ownerData?.contact_number || "N/A"}</p>
                            </div>
                            <div>
                                <p className="text-[10px] sm:text-xs text-muted-foreground uppercase tracking-wider mb-1 font-bold">Address</p>
                                <p className="text-xs sm:text-sm text-foreground font-medium leading-snug whitespace-normal">{shopData.address || "N/A"}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-card border border-border rounded-xl overflow-hidden shadow-sm">
                        ...
                        <div className="p-4 sm:p-5 space-y-4">
                            <div>
                                <p className="text-[10px] sm:text-xs text-muted-foreground uppercase tracking-wider mb-1 font-bold">Full Name</p>
                                <p className="text-xs sm:text-sm font-bold text-foreground">{ownerData?.full_name || "N/A"}</p>
                            </div>
                            <div>
                                <p className="text-[10px] sm:text-xs text-muted-foreground uppercase tracking-wider mb-1 font-bold">Email</p>
                                <p className="text-xs sm:text-sm text-foreground font-medium break-all whitespace-normal">{ownerData?.email || "N/A"}</p>
                            </div>
                            <div>
                                <p className="text-[10px] sm:text-xs text-muted-foreground uppercase tracking-wider mb-1 font-bold">Contact Number</p>
                            ...
                            <div>
                                <p className="text-[10px] sm:text-xs text-muted-foreground uppercase tracking-wider mb-1 font-bold">Joined Date</p>
                ...
                <div className="lg:col-span-2 space-y-6 sm:space-y-8">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div className="bg-card border border-border p-4 sm:p-5 rounded-xl shadow-sm relative overflow-hidden group">
                            ...
                            <p className="text-2xl sm:text-3xl font-bold text-foreground font-mono">{metrics.inventoryCount}</p>
                        </div>
                        <div className="bg-card border border-border p-4 sm:p-5 rounded-xl shadow-sm relative overflow-hidden group">
                            ...
                            <p className="text-2xl sm:text-3xl font-bold text-foreground font-mono">{metrics.staffCount}</p>
                        </div>
                        <div className="bg-card border border-primary/30 p-4 sm:p-5 rounded-xl shadow-sm relative overflow-hidden group">
                            ...
                            <p className="text-2xl sm:text-3xl font-bold text-foreground font-mono">₹{metrics.totalRevenue.toLocaleString('en-IN')}</p>
                        </div>
                    </div>

                    {/* Subscription & Billing Manual Override */}
                    <div className="bg-card border border-primary/30 rounded-xl overflow-hidden shadow-sm relative">
                        ...
                        <div className="px-4 sm:px-5 py-3.5 sm:py-4 border-b border-border bg-muted/20 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                            <div className="flex items-center gap-2 sm:gap-3">
                                <CreditCard className={`w-4 h-4 sm:w-5 sm:h-5 shrink-0 ${isRejected ? 'text-muted-foreground' : 'text-primary'}`} />
                                <h2 className="font-bold text-foreground text-sm sm:text-base">Subscription Governance</h2>
                            </div>
                            <span className={`w-fit px-2.5 py-0.5 rounded text-[10px] font-mono font-bold tracking-wider uppercase border shadow-sm ${isRejected ? 'bg-muted text-muted-foreground border-border' :
                                shopData.plan === 'PRO' ? 'bg-primary/10 text-primary border border-primary/30' :
                        ...
                        <div className="p-4 sm:p-5">
                            ...
                            <div className="flex flex-col sm:flex-row flex-wrap gap-3 w-full">
                                <button
                                    ...
                                    className="w-full sm:w-auto px-4 py-2.5 bg-background border border-border text-foreground text-sm font-bold rounded-lg transition-colors hover:bg-muted disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer shadow-sm"
                                >
                                    Force Starter
                                </button>
                                <button
                                    ...
                                    className="w-full sm:w-auto px-4 py-2.5 bg-blue-500/10 border border-blue-500/30 text-blue-500 text-sm font-bold rounded-lg transition-colors hover:bg-blue-500/20 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer shadow-sm"
                                >
                                    Force Growth
                                </button>
                                <button
                                    ...
                                    className="w-full sm:w-auto px-4 py-2.5 bg-primary/10 border border-primary/30 text-primary text-sm font-bold rounded-lg transition-colors hover:bg-primary/20 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer shadow-sm"
                                >
                                    Force Pro
                                </button>
                            </div>
                    ...
                    {/* Database Inspector Tab Ecosystem */}
                    <div className="space-y-4">
                        <h3 className="text-base sm:text-lg font-bold text-foreground flex items-center gap-2">
                            <Database className="w-4 h-4 sm:w-5 sm:h-5 text-primary shrink-0" /> Database Inspector
                        </h3>

                        {/* Swipeable Tabs Row */}
                        <div className="overflow-x-auto custom-scrollbar p-1 bg-card border border-border rounded-xl w-full shadow-sm">
                            <div className="flex gap-2 whitespace-nowrap min-w-max">
                                <button
                                    onClick={() => setActiveTab("inventory")}
                                    className={`px-4 py-2 rounded-lg text-xs sm:text-sm font-bold transition-all cursor-pointer ${activeTab === "inventory" ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:text-foreground hover:bg-muted"}`}
                                >
                                    Inventory ({metrics.inventoryCount})
                                </button>
                                <button
                                    onClick={() => setActiveTab("staff")}
                                    className={`px-4 py-2 rounded-lg text-xs sm:text-sm font-bold transition-all cursor-pointer ${activeTab === "staff" ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:text-foreground hover:bg-muted"}`}
                                >
                                    Staff ({metrics.staffCount})
                                </button>
                                <button
                                    onClick={() => setActiveTab("bills")}
                                    className={`px-4 py-2 rounded-lg text-xs sm:text-sm font-bold transition-all cursor-pointer ${activeTab === "bills" ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:text-foreground hover:bg-muted"}`}
                                >
                                    Recent Bills
                                </button>
                                <button
                                    onClick={() => setActiveTab("invoices")}
                                    className={`px-4 py-2 rounded-lg text-xs sm:text-sm font-bold transition-all cursor-pointer ${activeTab === "invoices" ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:text-foreground hover:bg-muted"}`}
                                >
                                    Payment Ledger
                                </button>
                            </div>
                                            ...
                                            {inventoryList.length === 0 ? (
                                                ...
                                                inventoryList.map((item) => (
                                                    <tr key={item.id} className="hover:bg-muted/50 transition-colors">
                                                        <td className="px-4 sm:px-6 py-3.5 font-bold text-foreground text-xs sm:text-sm">
                                                            {item.medicine_name}
                                                            <div className="text-[9px] sm:text-[10px] text-muted-foreground font-mono mt-0.5 font-medium">Batch: {item.batch_number}</div>
                                                        </td>
                                                        <td className="px-4 sm:px-6 py-3.5 text-xs sm:text-sm text-muted-foreground font-medium">{item.category}</td>
                                                        <td className="px-4 sm:px-6 py-3.5 text-center text-xs sm:text-sm font-mono text-foreground font-medium">{item.quantity}</td>
                                                        <td className="px-4 sm:px-6 py-3.5 text-right text-xs sm:text-sm text-emerald-500 font-mono font-bold">₹{item.mrp}</td>
                                                    </tr>
                                                ))
                                            )}
                                        </tbody>
                                            ...
                                            {staffList.length === 0 ? (
                                                ...
                                                staffList.map((staff) => (
                                                    <tr key={staff.id} className="hover:bg-muted/50 transition-colors">
                                                        <td className="px-4 sm:px-6 py-3.5 font-bold text-foreground text-xs sm:text-sm">{staff.name}</td>
                                                        <td className="px-4 sm:px-6 py-3.5 text-xs sm:text-sm text-muted-foreground font-medium">{staff.email}</td>
                                                        ...
                                                        <td className="px-4 sm:px-6 py-3.5">
                                                            <div className="flex items-center gap-1.5 text-xs sm:text-sm font-bold">
                                                                <span className={`w-1.5 h-1.5 rounded-full ${staff.status === 'ACTIVE' ? 'bg-primary' : 'bg-destructive'}`} />
                        ...
                        {/* Billing History Table Workspace */}
                        {activeTab === "bills" && (
                            <div className="bg-card border border-border rounded-xl overflow-hidden shadow-sm animate-in fade-in duration-300">
                                <div className="overflow-x-auto max-h-[400px] custom-scrollbar">
                                    <table className="w-full text-left border-collapse whitespace-nowrap min-w-[650px]">
                                            ...
                                            {billsList.length === 0 ? (
                                                ...
                                                billsList.map((bill) => (
                                                    <tr key={bill.id} className="hover:bg-muted/50 transition-colors">
                                                        <td className="px-4 sm:px-6 py-3.5 text-xs sm:text-sm text-muted-foreground font-mono font-medium">{formatDateTime(bill.created_at)}</td>
                                                        <td className="px-4 sm:px-6 py-3.5 font-bold text-foreground text-xs sm:text-sm">{bill.customer_name || "Guest"}</td>
                                                        <td className="px-4 sm:px-6 py-3.5 text-muted-foreground uppercase text-[10px] font-bold">{bill.payment_method || "N/A"}</td>
                                                        <td className="px-4 sm:px-6 py-3.5 text-right text-xs sm:text-sm font-bold text-emerald-500 font-mono">₹{bill.total_amount}</td>
                                                    </tr>
                                                ))
                                            )}
                                        </tbody>
                        ...
                        {/* Payment Ledger Stub Block */}
                        {activeTab === "invoices" && (
                            <div className="bg-card border border-border rounded-xl overflow-hidden shadow-sm animate-in fade-in duration-300 p-6 sm:p-12 flex flex-col items-center justify-center text-center">
                                <Receipt className="w-10 h-10 sm:w-12 sm:h-12 text-muted-foreground/30 mb-3" />
                                <h4 className="text-base sm:text-lg font-bold text-foreground mb-2">Payment Ledger Coming Soon</h4>
                                <p className="text-xs sm:text-sm text-muted-foreground max-w-md font-medium px-2 leading-relaxed">
