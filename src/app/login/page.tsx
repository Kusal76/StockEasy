...
export default function LoginPage() {
    ...
    const handleLogin = async (e?: React.FormEvent) => {
        ...
        try {
            ...
            // Admins are ALWAYS forced to set up 2FA. Owners are forced if the global toggle is ON.
            ...
            // Admins are ALWAYS challenged for a code. Owners are challenged if the global toggle is ON.
            if (hasVerifiedTOTP && (isAdmin || (userRole === "OWNER" && requiresGlobal2FA))) {
                setShow2FA(true);
                setIsLoading(false);
                return;
    ...
    return (
        <div className="h-screen w-full bg-background flex transition-colors duration-300 overflow-hidden">
            ...
            <div className="hidden lg:flex lg:w-1/2 relative bg-card border-r border-border flex-col justify-between p-12 h-full transition-colors duration-300">
                ...
                <div className="relative z-10">
                    <p className="text-sm text-muted-foreground font-medium">© 2026 StockEasy Technologies. All rights reserved.</p>
                </div>
            </div>

            {/* RIGHT COLUMN: Tabbed Login Form */}
            <div className="w-full lg:w-1/2 h-full overflow-y-auto flex flex-col p-6 sm:p-12 relative">
                ...
                <div className="w-full max-w-md m-auto bg-card sm:bg-transparent border border-border sm:border-0 rounded-2xl p-6 sm:p-0 animate-in fade-in slide-in-from-bottom-4 duration-500 shadow-sm sm:shadow-none transition-colors">
                    ...
                    {!show2FA && !show2FASetup && (
                        <>
                            <div className="flex mb-8 border-b border-border">
                                <button type="button" onClick={() => setLoginMode("portal")} className={`flex-1 pb-3 text-sm font-bold transition-colors cursor-pointer ${loginMode === "portal" ? "text-primary border-b-2 border-primary" : "text-muted-foreground hover:text-foreground"}`}>
                                    Pharmacy Portal
                                </button>
                                <button type="button" onClick={() => setLoginMode("admin")} className={`flex-1 pb-3 text-sm font-bold transition-colors cursor-pointer ${loginMode === "admin" ? "text-primary border-b-2 border-primary" : "text-muted-foreground hover:text-foreground"}`}>
                                    System Admin
                                </button>
                            </div>
                            ...
                            <div className="hidden sm:flex flex-col items-center justify-center mb-6">
                                ...
                                <p className="text-muted-foreground text-sm mt-2 flex items-center gap-2 font-medium">
                                    {loginMode === "portal" ? <><Users className="w-4 h-4 text-primary" /> Secure login for Owners & Staff</> : <><ShieldCheck className="w-4 h-4 text-primary" /> Central administrative access</>}
                                </p>
                            </div>
                            ...
                            {errorMsg && (
                                <div className="mb-6 p-4 bg-destructive/10 border border-destructive/30 rounded-lg flex items-center gap-3 animate-in fade-in slide-in-from-top-2 shadow-sm">
                            ...
                            {loginMode === "portal" && !maintenanceMode && (
                                <div>
                                    <button type="button" onClick={handleGoogleLogin} disabled={isLoading || isCheckingSession} className="w-full flex items-center justify-center gap-3 bg-card border border-border text-foreground font-bold py-3.5 rounded-xl hover:bg-muted transition-all disabled:opacity-70 disabled:cursor-not-allowed cursor-pointer shadow-sm">
                                    ...
                                    <div className="relative flex items-center py-5">
                                        ...
                                        <span className="flex-shrink-0 mx-4 text-muted-foreground text-xs font-bold uppercase tracking-wider">Or email & password</span>
                                        <div className="flex-grow border-t border-border"></div>
                                    </div>
                                </div>
                            )}
                            ...
                            <form action="#" method="POST" className="space-y-5">
                                ...
                                {loginMode === "admin" && (
                                    <div className="bg-primary/5 border border-primary/20 rounded-xl p-4 flex gap-3 mt-4 shadow-sm">
                                        <ShieldCheck className="w-5 h-5 text-primary shrink-0" />
                                        <p className="text-xs text-muted-foreground leading-relaxed font-medium">System Administrators must use their hardware security key (U2F) after providing credentials.</p>
                                    </div>
                                )}
                                ...
                                <button
                                    ...
                                    className="w-full flex items-center justify-center gap-2 bg-primary text-primary-foreground font-bold py-3.5 rounded-xl hover:bg-primary/90 transition-all shadow-sm mt-6 disabled:opacity-70 disabled:cursor-not-allowed cursor-pointer"
                                >
                                    {isLoading ? <><Loader2 className="w-5 h-5 animate-spin" /> Authenticating...</> : <>Login <ArrowRight className="w-4 h-4" /></>}
                                </button>
                            </form>
                    ...
                    {show2FASetup && (
                        <div className="animate-in slide-in-from-right-8 duration-500">
                            <div className="text-center mb-6">
                                <div className="w-16 h-16 bg-primary/10 border border-primary/20 rounded-full flex items-center justify-center mx-auto mb-4 shadow-sm">
                                    <QrCode className="w-8 h-8 text-primary" />
                                </div>
                                <h2 className="text-2xl font-bold text-foreground mb-2">Secure Your Account</h2>
                                <p className="text-muted-foreground text-sm font-medium px-4">As a Pharmacy Owner, you are required to secure your account with Two-Factor Authentication before accessing the dashboard.</p>
                            </div>
                            ...
                            <div className="bg-card border border-border p-5 rounded-2xl shadow-sm mb-6">
                                <h3 className="text-sm font-bold text-foreground mb-3 text-center">Step 1: Scan this QR Code</h3>
                                <div className="bg-white p-3 rounded-xl mx-auto w-fit mb-4 shadow-sm" dangerouslySetInnerHTML={{ __html: qrCodeSvg }} />

                                <form onSubmit={handleVerifySetup} className="space-y-4 pt-4 border-t border-border">
                                    <div className="space-y-2">
                                        <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider text-center block">Step 2: Enter 6-Digit Code</label>
                                        <input
                                            type="text"
                                            maxLength={6}
                                            value={otpCode}
                                            ...
                                            disabled={isLoading}
                                            className="w-full bg-background border border-border rounded-xl px-4 py-3 text-foreground text-xl font-mono text-center tracking-[0.5em] focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all disabled:opacity-50 shadow-sm"
                                            placeholder="000000"
                                            autoFocus
                                        />
                                    </div>

                                    <button type="submit" disabled={isLoading || otpCode.length !== 6} className="w-full flex items-center justify-center gap-2 bg-primary text-primary-foreground font-bold py-3 rounded-xl hover:bg-primary/90 transition-all shadow-sm disabled:opacity-70 disabled:cursor-not-allowed cursor-pointer">
                                        {isLoading ? <><Loader2 className="w-4 h-4 animate-spin" /> Verifying...</> : <>Secure & Enter Dashboard <ArrowRight className="w-4 h-4" /></>}
                                    </button>
                                </form>
                            </div>

                            <button type="button" onClick={async () => { await supabase.auth.signOut(); setShow2FASetup(false); }} className="w-full text-muted-foreground hover:text-foreground text-sm font-bold transition-colors cursor-pointer">
                                Cancel and sign out
                            </button>
                        </div>
                    )}
                    ...
                    {show2FA && !show2FASetup && (
                        <div className="animate-in slide-in-from-right-8 duration-500">
                            <div className="text-center mb-8">
                                ...
                                <p className="text-muted-foreground text-sm max-w-sm mx-auto font-medium">Your account role requires enhanced security. Please enter the 6-digit code from your Authenticator app.</p>
                            </div>

                            {errorMsg && (
                                <div className="mb-6 p-4 bg-destructive/10 border border-destructive/30 rounded-lg flex items-center gap-3 animate-in fade-in shadow-sm">
                                    <AlertCircle className="w-5 h-5 text-destructive flex-shrink-0" />
                                    <p className="text-sm font-bold text-destructive">{errorMsg}</p>
                                </div>
                            )}
                            ...
                            <form onSubmit={handleVerify2FA} className="space-y-6">
                                <div className="space-y-2">
                                    <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider text-center block">Authenticator Code</label>
                                    <input
                                        type="text"
                                        maxLength={6}
                                        value={otpCode}
                                        onChange={(e) => setOtpCode(e.target.value.replace(/\D/g, ''))}
                                        disabled={isLoading}
                                        className="w-full bg-background border border-border rounded-xl px-4 py-4 text-foreground text-2xl font-mono text-center tracking-[0.5em] focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all disabled:opacity-50 shadow-sm"
                                        placeholder="000000"
                                        autoFocus
                                    />
                                </div>

                                <button type="submit" disabled={isLoading || otpCode.length !== 6} className="w-full flex items-center justify-center gap-2 bg-primary text-primary-foreground font-bold py-3.5 rounded-xl hover:bg-primary/90 transition-all shadow-sm disabled:opacity-70 disabled:cursor-not-allowed cursor-pointer">
                                    {isLoading ? <><Loader2 className="w-5 h-5 animate-spin" /> Verifying Token...</> : <>Verify & Secure Login <ShieldCheck className="w-4 h-4" /></>}
                                </button>

                                <button type="button" onClick={async () => { await supabase.auth.signOut(); setShow2FA(false); }} className="w-full text-muted-foreground hover:text-foreground text-sm font-bold mt-4 transition-colors cursor-pointer">
